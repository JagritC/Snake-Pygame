...
class Cube(object):
    ...
    def draw(self, surface, eyes=False):
        ...
        if eyes:
            center = dist //2
            radius = 3
            eye1 = (i*dist+center-radius, j*dist+8)
        ...
        for event in pygame.event.get():
            ...
            for key in keys:
                if keys[pygame.K_LEFT]:
                    ...
                    self.dirnx = 1  # moving right
                    self.dirny = 0
                    self.turns[self.head.pos[:]] = [self.dirnx, self.dirny]
                elif keys[pygame.K_UP]:
                    self.dirnx = 0  # moving up
                    self.dirny = -1
                    self.turns[self.head.pos[:]] = [self.dirnx, self.dirny]
                elif keys[pygame.K_DOWN]:
                    self.dirnx = 0  # moving down
                    self.dirny = 1
                    self.turns[self.head.pos[:]] = [self.dirnx, self.dirny]
        ...
        for i, c in enumerate(self.body):
            ...
            if p in self.turns:
                ...
                if i == len(self.body) - 1:
                    self.turns.pop(p)
            else: ## if hit wall come around the other side
